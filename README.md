...
следующие типа данные: String, LocalDate, OffsetDateTime, Long, Double, Integer, BigDecimal а также эти типы данных 
в качестве элементов коллекций Map, List, Set, Array. Идентификатор LOCALDATE работает только с полями типа LocalDate 
(и коллекциями с ними), идентификатор OFFSET_DATE_TIME работает только с полями типа OffsetDateTime (и коллекциями с ними), 
идентификатор BALANCE работает только с полями типа Long, Double, Integer, BigDecimal (и коллекциями с ними), остальные 
идентификаторы работают только с полями типа String. Идентификаторы работают нормально с полями типа Object, если из него 
...
7) Для использования программы нужно использовать один из 3 статических методов из класса Masker в классе, где требуется обработка ДТО:

**mask** - примет экземпляр ДТО с незамаскированными данными и вернет другой экземпляр этого ДТО с замаскированными данными. 
Значения полей типов, отличных от String, маскируются не на звездочки, а на условные значения - например в датах годменяется на нулевой.

**maskToString** - примет экземпляр ДТО с незамаскированными данными и вернет замаскированное ДТО в виде String. Все нужные значения будут 
будут замаскированы звездочками. Этот метод сначала отправляет полученное ДТО в **mask**, потом переводит в String полученный 
результат и *затем* меняет условные замаскированные значения в полях, отличных от String, на звездочки. Для работы метода, 
в ДТО (на всех уровнях вложенности) должен быть переопределен toString. Чтобы условные замаскированные значения корректно заменялись 
на звездочки, у переопределения toString даты должны быть в формате YYYY-MM-DD.

**maskToStringWithOverride** - примет экземпляр ДТО с незамаскированными данными и вернет замаскированное ДТО в виде String. Все нужные значения будут
будут замаскированы звездочками. Этот метод сначала отправляет полученное ДТО в **mask**, потом переводит в String полученный
результат и *одновременно* меняет условные замаскированные значения в полях, отличных от String, на звездочки. Этот метод не 
зависит от переопределения toString.


Если нужно получить именно ДТО (например для доступа к полям), рекомендуется использовать **mask**. Если нужно чтобы везде в качестве маскировки были  
звездочки (а не условные значения в полях где тип не дает поставить звездочку), рекомендуется использовать **maskToString**. 
Если при этом в ДТО не везде есть переопределенный toString, рекомендуется использовать **maskToStringWithOverride**.
...
енум-идентификатор.
8. class **DtoToStringProcessor** - отвечает за перевод замаскированного ДТО в String.
